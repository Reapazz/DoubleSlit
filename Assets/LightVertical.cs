        ...
        { //If the remainder of the current frame divided by 100 is 0 run the function.
...

            {
            ...
            else foreach (UnityEngine.Collider obj in intersecting)
                {
                    ...
                    else if (obj.gameObject.GetComponent<Move1>() != null || obj.gameObject.GetComponent<LightVertical>() != null) //only other possible option is that it collided with another light particle which is not entangled in same direction
                    {
